...
class HPOpt(object):
    ...
    def train_reg(self, reg, para):
        reg.fit(self.x_train, self.y_train,
                eval_set= [(self.x_test, self.y_test)],  #[(self.x_train, self.y_train),(self.x_test, self.y_test)],
                **para['fit_params'])
        y_pred_probab_train = reg.predict_proba(self.x_train)[:, 1]    
        y_pred_probab = reg.predict_proba(self.x_test)[:, 1]    
        
        train_auc = metrics.roc_auc_score(self.y_train, y_pred_probab_train)
        val_auc = metrics.roc_auc_score(self.y_test, y_pred_probab)
